...
public Action MyServiceMenu(int User, int Arg)
{
	...
	if(QueryDB != INVALID_HANDLE)
	{
		...
		while(SQL_FetchRow(QueryDB))
		{
			...
			} else { SQL_GetError(DB, Error, sizeof(Error)); LogToFile("addons/sourcemod/logs/OnlineShop.txt", "[MySQL MyServiceMenu #1] %s", Error); }
		}
		...
		AddMenuItem(OpenMenu, "back", "Powrót");
		DisplayMenu(OpenMenu, User, 250);
	} else { SQL_GetError(DB, Error, sizeof(Error)); LogToFile("addons/sourcemod/logs/OnlineShop.txt", "[MySQL MyServiceMenu #2] %s", Error); }
}
